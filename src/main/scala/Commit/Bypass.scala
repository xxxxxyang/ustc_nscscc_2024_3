...
class Bypass(n: Int) extends Module {
    ...
    for(i <- 0 until 3){
        val hit_j = VecInit.tabulate(2)(j => (io.rd_valid_wb(j) && io.prd_wb(j) === io.prj_ex(i)))
        ...
        val hit_k = VecInit.tabulate(2)(j => (io.rd_valid_wb(j) && io.prd_wb(j) === io.prk_ex(i)))
    ...
    //LS
    val hit_j = VecInit.tabulate(3)(j => (io.rd_valid_wb(j) && io.prd_wb(j) === io.prj_ex(3)))
    io.forward_prj_en(3) :=  hit_j.asUInt.orR
    io.forward_prj_data(3) := Mux1H(hit_j, io.prd_wdata_wb)

    val hit_k = VecInit.tabulate(3)(j => (io.rd_valid_wb(j) && io.prd_wb(j) === io.prk_ex(3)))
