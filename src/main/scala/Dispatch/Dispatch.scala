...
class Dispatch(n: Int) extends Module {
  ...
  for(i <- 0 until 2){
    queue_hit(i) := UIntToOH(Mux(io.inst_pack(i).fu_id(2).asBool, Mux(io.elem(1) <= io.elem(2), 1.U(2.W), 0.U(2.W)), io.inst_pack(i).fu_id(1,0))) & Fill(4, io.inst_pack(i).inst_valid)
  }
