...
class FreeList(n:Int) extends Module {
    val io = IO(new Bundle {
        ...
        val commmit_en = Input(Vec(2,Bool()))
        val commit_pprd_valid = Input(Vec(2,Bool()))
        val commit_pprd = Input(Vec(2,log2Ceil(n).W))
    ...
    for(i <- 0 until 2){
        ...
        head_temp = Mux(io.rename_en(i)&io.rd_valid(i)&!empty,head_temp(n-2,0)##head_temp(n-1),head_temp)
        io.alloc_preg(i) := Mux1H(head+sel(i),queue)
    }
